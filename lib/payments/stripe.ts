...
export async function createCustomerPortalSession(user: User) {
  ...
  if (configurations.data.length > 0) {
    ...
    if (!product.active) {
      throw new Error("Produto do usuário não está ativo no Stripe");
    }
    ...
    if (prices.data.length === 0) {
      throw new Error("Nenhum preço ativo encontrado para o produto do usuário");
    }
...
export async function handleSubscriptionChange(
  ...
  if (!user) {
    console.error('Usuário não encontrado para cliente Stripe:', customerId);
    return;
  }
