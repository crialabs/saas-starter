...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  if (existingUser.length > 0) {
    return { error: 'Falha ao criar usuário. Por favor, tente novamente.' };
  }
...
export const deleteAccount = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    if (!isPasswordValid) {
      return { error: 'Senha incorreta. A exclusão da conta foi cancelada.' };
    }
    ...
    await db
      .update(users)
      .set({ deletedAt: sql`CURRENT_TIMESTAMP`, updatedAt: new Date() })
      .where(eq(users.id, user.id));
